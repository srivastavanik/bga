...
function fixChatFunctionality() {
  ...
  const thinkingProcess = document.querySelector(
    '.ThinkingProcess, [class*="thinking"]'
  );
  ...
  if (!messageContainer) {
    const existingContainer = thinkingProcess.querySelector(
    ...
    if (existingContainer) {
      ...
      const headerElem = thinkingProcess.querySelector(
        'h5, h6, [class*="header"]'
      );
      if (headerElem) {
        ...
        messageContainer.style.cssText =
          "margin-top:16px;max-height:500px;overflow-y:auto;border:1px solid rgba(0,0,0,0.1);border-radius:4px;padding:16px;background-color:#1a1a1d;color:#fff;";
        headerElem.parentNode.insertBefore(
          messageContainer,
          headerElem.nextSibling
        );
      }
    }
  }
  ...
  if (!inputContainer) {
    ...
    inputContainer.style.cssText =
      "display:flex;margin-top:16px;padding:8px;background-color:rgba(255,255,255,0.05);border-radius:4px;";

    const textarea = document.createElement("textarea");
    textarea.className = "chat-input";
    textarea.placeholder = "Ask a follow-up question or request refinements...";
    textarea.style.cssText =
      "flex:1;border:none;background-color:transparent;color:#fff;resize:none;padding:8px;min-height:40px;font-family:inherit;";

    const sendButton = document.createElement("button");
    sendButton.className = "send-button";
    sendButton.innerHTML =
      '<svg viewBox="0 0 24 24" width="24" height="24" fill="#2196f3"><path d="M2.01 21L23 12 2.01 3 2 10l15 2-15 2z"></path></svg>';
    sendButton.style.cssText =
      "background:none;border:none;color:#2196f3;cursor:pointer;display:flex;align-items:center;justify-content:center;";
...
function extractThinkingContent(container) {
  ...
  const contentSections = container.querySelectorAll(
    '[class*="content"], [class*="details"], pre, [class*="body"]'
  );
  ...
  apiPromise
    ...
    .catch((error) => {
      ...
      errorMessage.style.cssText =
        "color:#f44336;padding:10px;margin-bottom:16px;";
