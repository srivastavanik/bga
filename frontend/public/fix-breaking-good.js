...
function fixMoleculeVisualization() {
  ...
  const moleculeContainers = document.querySelectorAll(
    '.viewerContainer, [class*="viewerContainer"]'
  );
  ...
  moleculeContainers.forEach(function (container, index) {
    ...
    console.log(
      `Container ${index}: Found SMILES ${smiles.substring(0, 20)}...`
    );
...
function findSmilesNearContainer(container) {
  ...
  const parent =
    container.closest(
      '[class*="Card"], [class*="cardContent"], [class*="Card"]'
    ) || container.parentElement;

  // Look for elements that might contain SMILES
  const possibleElements = parent.querySelectorAll(
    'code, pre, [class*="codeBlock"], .MuiTypography-body2, div'
  );
...
function createMoleculeVisualization(container, smiles) {
  try {
    ...
    const viewerId =
      "molecule-viewer-" + Math.random().toString(36).substring(2, 9);
...
function fixSaveAndSelectButtons() {
  ...
  const saveButtons = Array.from(
    document.querySelectorAll('button[class*="save"], button')
  ).filter((button) => button.textContent.toLowerCase().includes("save"));
  const selectButtons = Array.from(
    document.querySelectorAll('button[class*="select"], button')
  ...
  saveButtons.forEach(function (button) {
    ...
    button.addEventListener("click", function (event) {
      ...
      const container = button.closest(
        '[class*="Card"], [class*="cardContent"], [class*="molecule"]'
      );
      if (!container) return;
      ...
      try {
        const savedMolecules = JSON.parse(
          localStorage.getItem("savedMolecules") || "[]"
        );
  ...
  selectButtons.forEach(function (button) {
    ...
    button.addEventListener("click", function (event) {
      ...
      const mainViewer = document.querySelector(
        '.MoleculeViewer3D, [class*="MoleculeViewer3D"]'
      );
      if (mainViewer) {
...
function fixChatFunctionality() {
  ...
  const chatInputs = document.querySelectorAll(
    'textarea[placeholder*="question"], textarea[placeholder*="refinements"], textarea[class*="input"]'
  );
  const sendButtons = Array.from(
    document.querySelectorAll('button[class*="send"], button')
  ).filter((button) => {
    const buttonText = button.textContent.toLowerCase();
    return (
      buttonText.includes("send") || button.querySelector('svg[class*="Send"]')
    );
  });
  ...
  sendButtons.forEach(function (button) {
    ...
    button.addEventListener("click", function (event) {
      ...
      const container = button.closest(
        '[class*="input"], [class*="chat"], [class*="message"]'
      );
      if (!container) return;
...
function sendChatMessage(message) {
  ...
  const messageContainer = document.querySelector(
    '[class*="messageContainer"], [class*="messages"], [class*="chat"]'
  );
  ...
  setTimeout(function () {
    ...
    let response;
    if (
      message.toLowerCase().includes("properties") ||
