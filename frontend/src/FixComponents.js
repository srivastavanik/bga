    ...
    if (typeof window.handleSelectFromThinking === "undefined") {
      window.handleSelectFromThinking = async function (smiles) {
        ...
        const newMolecule = {
          id: Date.now(),
          name: `AI-Compound-${
            window.aiGeneratedMolecules
              ? window.aiGeneratedMolecules.length + 1
              : 1
          }`,
          smiles: smiles,
        ...
        try {
          if (window.simulationAPI && window.simulationAPI.runRDKit) {
            ...
            newMolecule.properties = {
              molecularWeight: `${propertiesResponse.data.molecular_weight.toFixed(
                2
              )} g/mol`,
            ...
            if (window.setAiGeneratedMolecules) {
              window.setAiGeneratedMolecules((prev) =>
                prev.map((m) =>
                  m.id === newMolecule.id
                    ? { ...m, properties: newMolecule.properties }
                    : m
                )
              );
            }
  ...
  window.fixVisualization = function () {
    ...
    const fixViewers = function () {
      ...
      if (viewerContainers.length > 0) {
        console.log(
          `Found ${viewerContainers.length} molecular viewers to fix`
        );
        ...
        viewerContainers.forEach((container, index) => {
          ...
          const smilesElements = parent.querySelectorAll(
  ...
  window.exposeInternals = function () {
    ...
    const findReactComponents = function () {
      ...
      const rootElements = document.querySelectorAll(
        '#root, [class*="MoleculeDesigner"], [class*="ThinkingProcess"]'
      );
          ...
          if (
            ...
            if (fiberNode && fiberNode.return) {
              ...
              while (node) {
                if (
                  node.stateNode &&
                  node.stateNode.constructor &&
                  node.stateNode.constructor.name
                ) {
                  const name = node.stateNode.constructor.name;
                  if (name.includes("MoleculeDesigner")) {
                    console.log(
                      "Found MoleculeDesigner component",
                      node.stateNode
                    );
                      ...
                      window.setAiGeneratedMolecules = (molecules) => {
                        node.stateNode.setState({
                          aiGeneratedMolecules:
                            typeof molecules === "function"
                              ? molecules(
                                  node.stateNode.state.aiGeneratedMolecules
                                )
                              : molecules,
                        });
                      };
                    ...
                    console.log(
                      "Found ThinkingProcess component",
                      node.stateNode
                    );
