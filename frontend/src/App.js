...
const landingTheme = createTheme({
  ...
  overrides: {
    ...
    MuiButton: {
      ...
      outlinedPrimary: {
        // This corresponds to the secondary button style
        borderColor: "#111111",
        color: "#111111",
    ...
    MuiOutlinedInput: {
      root: {
        ...
        "&$notchedOutline": {
          // Correct way to target the notch
          borderColor: "#EAE8E4",
        },
...
function App() {
  ...
  useEffect(() => {
    ...
    if (savedNotes) {
      try {
        ...
        // Ensure loaded notes have proper Date objects if stored as strings
        const notesWithDates = parsedNotes.map((note) => ({
          ...note,
          createdAt: note.createdAt ? new Date(note.createdAt) : new Date(),
        }));
        setGlobalNotes(notesWithDates);
      } catch (error) {
        console.error("Error parsing global notes from localStorage:", error);
        localStorage.removeItem("globalNotes"); // Clear corrupted data
      }
    }
  }, []);
  ...
  const addGlobalNote = (note) => {
    if (note.title && note.content) {
      ...
      setGlobalNotes((prevNotes) => [...prevNotes, newNote]);
    } else {
      console.warn("Attempted to add an incomplete note:", note);
    }
  };

  const deleteGlobalNote = (id) => {
    setGlobalNotes((prevNotes) => prevNotes.filter((note) => note.id !== id));
  };
        ...
        <div className={classes.root}>
          <AppBar position="fixed" className={classes.appBar}>
            <Toolbar>
              ...
              <Button color="inherit" onClick={() => setShowLandingPage(true)}>
                Home
              </Button>
            </Toolbar>
          </AppBar>
          ...
          <main className={classes.content}>
            ...
            <Container className={classes.tabPanel}>
              ...
              <TabPanel value={tabValue} index={3}>
                <LiteratureExplorer addGlobalNote={addGlobalNote} />
              </TabPanel>
          ...
          <GlobalNotes
            notes={globalNotes}
            onAddNote={addGlobalNote}
            onDeleteNote={deleteGlobalNote}
          />
        </div>
      )}
    </ThemeProvider>
