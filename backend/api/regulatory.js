  ...
  try {
    ...
    try {
      ...
      return res
        .status(500)
        .json({
          error: "Failed to calculate molecule properties",
    ...
    try {
      ...
      const similarityResponse = await axios.post(
        "http://localhost:5001/api/similarity/search",
        {
          query: smiles,
      ...
      logger.warn(
        "Similar drug search currently uses general ChEMBL similarity, not specific approved drug database."
      );
      // If the similarity endpoint returns results:
      // similarDrugs = similarityResponse.data?.filter(d => d.pref_name).slice(0, 5) || [];
      logger.info(
        `Found ${similarDrugs.length} potentially similar ChEMBL compounds.`
      );
    } catch (error) {
...
Provide realistic estimations and justifications based *only* on the provided data. Acknowledge limitations where data is missing.`;
    ...
    try {
      ...
      logger.info(
        "Successfully generated regulatory analysis report via Claude."
      );
    } catch (error) {
      logger.error(
        `Error calling Claude for regulatory analysis: ${error.message}`
      );
    ...
    logger.error(`Regulatory report generation error: ${error.message}`, {
      stack: error.stack,
    });
...
router.post("/pathway", async (req, res) => {
  logger.warn(
    "Endpoint /api/regulatory/pathway called - DEPRECATED. Use /api/regulatory/report instead."
  );
  res
    .status(404)
    .json({
      error: "Endpoint deprecated. Use /api/regulatory/report instead.",
    });
});

router.post("/production-feasibility", async (req, res) => {
  logger.warn(
    "Endpoint /api/regulatory/production-feasibility called - DEPRECATED. Use /api/regulatory/report instead."
  );
  res
    .status(404)
    .json({
...
router.post("/market-analysis", async (req, res) => {
  logger.warn(
    "Endpoint /api/regulatory/market-analysis called - DEPRECATED. Use /api/regulatory/report instead."
  );
  res
    .status(404)
    .json({
