...
const ANTHROPIC_API_KEY =
  ...
  "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";
const ANTHROPIC_API_URL = "https://api.anthropic.com/v1/messages";
const ANTHROPIC_VERSION = "2023-06-01"; // Required anthropic-version header

// Log API key status (not the actual key)
logger.info(
  `Anthropic API Key status: ${ANTHROPIC_API_KEY ? "Loaded" : "Missing"}`
);
...
const requireApiKey = (req, res, next) => {
  ...
  if (ANTHROPIC_API_KEY) {
    ...
    logger.error(
      "CRITICAL: ANTHROPIC_API_KEY is not configured on the server."
    );
    return res
      .status(500)
...
const askClaude = async (
  ...
  try {
    ...
    while (true) {
      try {
        ...
        if (Array.isArray(response.data.content)) {
          logger.info(
            `Content is array with ${response.data.content.length} items`
          );
        ...
        if (isConnectionError) {
          ...
          logger.warn(
            `Connection error to Claude API. Retrying in ${
              waitTime / 1000
    ...
    if (error.response) {
      logger.error(
        `Claude API response: ${JSON.stringify(error.response.data)}`
      );
    }
...
const processMoleculeDesign = async (claudeResponse) => {
  try {
    ...
    if (typeof claudeResponse.content === "string") {
      ...
      logger.info(
        `Using string content directly, length: ${responseText.length}`
      );
    } else if (
      claudeResponse.rawContent &&
      Array.isArray(claudeResponse.rawContent)
    ) {
      ...
      logger.warn(
        "Attempting to extract text from unexpected response structure"
      );
      const findText = (obj) => {
    ...
    logger.info(
      `Extracted ${responseText.length} characters of text from Claude response`
    );
    let smilesCandidates = extractSMILES(responseText);
    logger.info(
      `Found ${smilesCandidates.length} SMILES candidates via regex.`
    );

    if (smilesCandidates.length === 0) {
      logger.warn(
        "No SMILES found via regex in Claude response, using fallback molecules"
      );
      smilesCandidates = FALLBACK_SMILES;
      logger.info(
        `Using ${smilesCandidates.length} fallback SMILES structures`
      );
    }

    const validatedMolecules = [];
    for (const smiles of [...new Set(smilesCandidates)]) {
      ...
      try {
        ...
        try {
          ...
          const propertiesResponse = await internalApiClient.post(
            "/api/simulation/properties",
            { smiles }
          );
          ...
          if (propertiesResponse.data && !propertiesResponse.data.error) {
            ...
            try {
              const structureResponse = await internalApiClient.post(
                "/api/simulation/3d-structure",
                { smiles }
              );
              if (structureResponse.data && structureResponse.data.molblock) {
                ...
                logger.info(
                  `Successfully validated SMILES (Props & 3D): ${smiles}`
                );
              } else {
                logger.warn(
                  `Failed 3D structure generation for valid SMILES: ${smiles} - ${
                    structureResponse.data?.error || "Unknown structure error"
                  }`
                );
                ...
                logger.info(
                  `Added molecule with properties but without 3D structure: ${smiles}`
                );
              }
            } catch (structureErr) {
              logger.warn(
                `Error during 3D structure call for SMILES ${smiles}: ${structureErr.message}`
              );
              // Still add the molecule if 3D structure failed but properties are valid
              validatedMolecules.push({
                smiles: smiles,
              ...
              logger.info(
                `Added molecule with properties but couldn't generate 3D structure: ${smiles}`
              );
            }
          } else {
            logger.warn(
              `Invalid SMILES structure or property calculation failed: ${smiles} - ${
                propertiesResponse.data?.error || "Unknown properties error"
              }`
            );
          }
        } catch (propertyErr) {
          ...
          logger.warn(
            `Error calculating properties for SMILES: ${smiles}. Error: ${propertyErr.message}`
          );
          ...
          if (
            ...
            logger.info(
              `Using basic validation for SMILES since RDKit failed: ${smiles}`
            );
            validatedMolecules.push({
              smiles: smiles,
              name: `Molecule Candidate ${validatedMolecules.length + 1}`,
            ...
            logger.info(
              `Added molecule with basic validation (RDKit unavailable): ${smiles}`
            );
          }
        }
      } catch (validationErr) {
        logger.error(
          `Error during validation/property call for SMILES ${smiles}: ${validationErr.message}`
        );
      }
    }

    logger.info(
      `Validated ${validatedMolecules.length} molecules (Props & 3D) out of ${smilesCandidates.length} candidates.`
    );
    if (validatedMolecules.length === 0) {
      logger.error(
        "No valid molecules could be validated from response or fallback."
      );
      throw new Error(
        "No valid molecules could be validated from Claude response or fallbacks"
      );
    }
...
router.post("/generate-molecule", async (req, res) => {
  try {
    ...
    logger.info(
      `Received molecule generation request with requirements: ${requirements}`
    );
    ...
    if (!requirements) {
      ...
      return res
        .status(400)
        .json({ error: "Molecule requirements are required" });
    }
    ...
    logger.info(
      `API Key Status: ${ANTHROPIC_API_KEY ? "Key is set" : "Key is missing"}`
    );
...
Provide at least 3 distinct candidate molecules with fully specified, valid SMILES strings.`;
    ...
    try {
      ...
      const useTestMode =
        ...
        (req.query && req.query.test === "true");

      if (useTestMode) {
        logger.info(
          "Using test mode (bypassing Claude API). This should be removed for production."
        );
      ...
      logger.error(
        `Error details: ${JSON.stringify(claudeError.response?.data || {})}`
      );
    ...
    try {
      ...
      logger.info(
        `Successfully extracted and validated ${generatedMolecules.length} molecules`
      );
    } catch (processError) {
      logger.error(
        `Failed to process Claude response: ${processError.message}`
      );
    ...
    const enhancedMolecules = await Promise.all(
      generatedMolecules.map(async (molecule) => {
        let admetData = null;
        try {
          const admetResponse = await internalApiClient.post(
            "/api/simulation/admet",
          ...
          if (admetResponse.data && !admetResponse.data.error) {
            ...
            logger.warn(
              `ADMET prediction failed for ${molecule.smiles}: ${admetResponse.data?.error}`
            );
          }
        } catch (err) {
          logger.error(
            `Error calling ADMET endpoint for ${molecule.smiles}: ${err.message}`
          );
        }
        return {
          ...molecule,
    ...
    logger.error(`Error in molecule generation endpoint: ${error.message}`, {
      stack: error.stack,
    });
    return res
      .status(500)
      .json({
        error: `Server error during molecule generation: ${error.message}`,
      });
  }
});
...
router.post("/analyze-molecules", async (req, res) => {
  try {
    ...
    if (!molecules || !Array.isArray(molecules) || molecules.length === 0) {
      return res
        .status(400)
        .json({ error: "At least one molecule is required" });
    }

    if (
      !targetReceptors ||
      !Array.isArray(targetReceptors) ||
    ...
    const moleculesFormatted = molecules
      .map((mol, index) => {
...
router.post("/optimize-molecule", async (req, res) => {
  try {
    ...
    if (!targetProperty) {
      return res
        .status(400)
        .json({ error: "Target property to optimize is required" });
    }
...
router.post("/compare-molecules", async (req, res) => {
  try {
    ...
    if (!molecules || !Array.isArray(molecules) || molecules.length < 2) {
      return res
        .status(400)
        .json({ error: "At least two molecules are required for comparison" });
    }
    ...
    const moleculesFormatted = molecules
      .map((mol, index) => {
        return `Molecule ${index + 1}:\n- SMILES: ${mol.smiles}\n- Name: ${
          ...
          mol.properties
            ? `\n- Properties: ${JSON.stringify(mol.properties)}`
            : ""
    ...
    if (criteria && criteria.length > 0) {
      ...
      userPrompt +=
        "Please compare these molecules on efficacy, safety, pharmacokinetics, and development potential.\n\n";
    }
...
router.post("/chat", requireApiKey, async (req, res) => {
  try {
    ...
    logger.info(
      `Received /chat request. Model: ${model}, Temp: ${temperature}, MaxTokens: ${max_tokens}`
    );
    ...
    res.status(error.response?.status || 500).json({
      error:
        "Failed to communicate with AI assistant. Please check server logs.",
