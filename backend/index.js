...
app.get("/api/health", (req, res) => {
  res
    .status(200)
    .json({ status: "up", message: "Breaking Good API is running" });
});
...
server.on("error", (e) => {
  if (e.code === "EADDRINUSE") {
    console.error(
      `Port ${PORT} is already in use. Please close the other application using this port or change the PORT in .env file.`
    );
    process.exit(1);
  } else {
